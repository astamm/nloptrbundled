...
static void update_(int *n, int *npt, double *bmat, double *zmat, int *ndim,
...
static nlopt_result rescue_(int *n, int *npt, const double *xl,
                            const double *xu,
                            /* int *maxfun */
                            nlopt_stopping *stop, bobyqa_func calfun,
                            void *calfun_data, double *xbase, double *xpt,
                            double *fval, double *xopt, double *gopt,
                            double *hq, double *pq, double *bmat, double *zmat,
                            int *ndim, double *sl, double *su, /* int *nf,  */
                            double *delta, int *kopt, double *vlag,
                            double *ptsaux, double *ptsid, double *w) {
  /* System generated locals */
  ...
  for (j = 1; j <= i__1; ++j) {
    ...
    if ((d__2 = ptsaux[(j << 1) + 2], fabs(d__2)) <
        half * (d__1 = ptsaux[(j << 1) + 1], fabs(d__1))) {
  ...
  for (j = 1; j <= i__2; ++j) {
    ...
    if (jpn <= *npt) {
      ...
      zmat[j * zmat_dim1 + 1] =
          sqrt(2.) /
          (d__1 = ptsaux[(j << 1) + 1] * ptsaux[(j << 1) + 2], fabs(d__1));
      zmat[jp + j * zmat_dim1] =
...
static void altmov_(int *n, int *npt, double *xpt, double *xopt, double *bmat,
...
static void trsbox_(int *n, int *npt, double *xpt, double *xopt, double *gopt,
                    double *hq, double *pq, double *sl, double *su,
                    double *delta, double *xnew, double *d__, double *gnew,
                    double *xbdi, double *s, double *hs, double *hred,
  ...
  int iterc;
  double resid, delsq, ggsav = 0.0, tempa, tempb, redmax, dredsq = 0.0, redsav,
                       onemin, gredsq = 0.0, rednew;
  int itcsav = 0;
...
static nlopt_result prelim_(int *n, int *npt, double *x, const double *xl,
                            const double *xu, double *rhobeg,
                            nlopt_stopping *stop, bobyqa_func calfun,
                            void *calfun_data, double *xbase, double *xpt,
                            double *fval, double *gopt, double *hq, double *pq,
                            double *bmat, double *zmat, int *ndim, double *sl,
                            double *su, int *kopt) {
  /* System generated locals */
  int xpt_dim1, xpt_offset, bmat_dim1, bmat_offset, zmat_dim1, zmat_offset,
      i__1, i__2;
  ...
  double one;
  int nfx, ipt = 0, jpt = 0;
  /* fbeg will be initialized below, init here to avoid maybe-uninitialized
   * warning */
  double two, fbeg = 0.0, diff, half, temp, zero, recip, stepa = 0.0,
              stepb = 0.0;
  int itemp;
  double rhosq;
...
static nlopt_result bobyqb_(int *n, int *npt, double *x, const double *xl,
                            const double *xu, double *rhobeg, double *rhoend,
                            nlopt_stopping *stop, bobyqa_func calfun,
                            void *calfun_data, double *minf, double *xbase,
                            double *xpt, double *fval, double *xopt,
                            double *gopt, double *hq, double *pq, double *bmat,
                            double *zmat, int *ndim, double *sl, double *su,
                            double *xnew, double *xalt, double *d__,
  ...
  rc2 = prelim_(n, npt, &x[1], &xl[1], &xu[1], rhobeg, stop, calfun,
                calfun_data, &xbase[1], &xpt[xpt_offset], &fval[1], &gopt[1],
...
L190:
  nfsav = *(stop->nevals_p);
  kbase = kopt;
  rc2 = rescue_(n, npt, &xl[1], &xu[1], stop, calfun, calfun_data, &xbase[1],
                &xpt[xpt_offset], &fval[1], &xopt[1], &gopt[1], &hq[1], &pq[1],
                &bmat[bmat_offset], &zmat[zmat_offset], ndim, &sl[1], &su[1],
...
L360:
  ...
  update_(n, npt, &bmat[bmat_offset], &zmat[zmat_offset], ndim, &vlag[1], &beta,
          &denom, &knew, &w[1]);
  ih = 0;
  pqold = pq[knew];
  pq[knew] = zero;
...
nlopt_result bobyqa(int n, int npt, double *x, const double *xl,
                    const double *xu, const double *dx, nlopt_stopping *stop,
                    double *minf, nlopt_func f, void *f_data) {
  /* System generated locals */
  int i__1;
  double d__1, d__2;

  /* Local variables */
  int j, id, np, iw, igo, ihq, ixb, ixa, ifv, isl, jsl, ipq, ivl, ixn, ixo, ixp,
      isu, jsu, ndim;
  double temp, zero;
  int ibmat, izmat;
  ...
  for (j = 1; j <= i__1; ++j) {
    ...
    if (temp < rhobeg + rhobeg) {
      ...
      ret = NLOPT_INVALID_ARGS;
      nlopt_stop_msg(stop,
                     "insufficient space between the bounds: %g - %g < %g",
  ...
  ret = bobyqb_(&n, &npt, &x[1], &xl[1], &xu[1], &rhobeg, &rhoend, stop,
                rescale_fun, &calfun_data, minf, &w[ixb], &w[ixp], &w[ifv],
                &w[ixo], &w[igo], &w[ihq], &w[ipq], &w[ibmat], &w[izmat], &ndim,
                &w[isl], &w[isu], &w[ixn], &w[ixa], &w[id], &w[ivl], &w[iw]);
