  ...
  *mode = 5;
  ...
  for (i__ = 1; i__ <= i__2; ++i__) {
    ...
    for (j = 1; j <= i__1; ++j) {
      ...
      g[i__ + j * g_dim1] =
          (g[i__ + j * g_dim1] -
           ddot_sl__(&i__3, &g[i__ + g_dim1], *lg, &e[j * e_dim1 + 1], 1)) /
          e[j + j * e_dim1];
    }
    /* L30: */
...
static void hfti_(double *a, int *mda, int *m, int *n, double *b, int *mdb,
  ...
  /* Parameter adjustments - use volatile intermediates to prevent GCC/ASAN
     from generating a false-positive -Warray-bounds diagnostic on the
     Fortran 1-based pointer arithmetic idiom produced by f2c.  A read from
     a volatile variable breaks GCC's pointer-provenance (VRP) tracking, so
     it cannot prove that the adjusted pointer is before the array start. */
  {
    int *volatile _v = ip;
    ip = _v - 1;
  }
