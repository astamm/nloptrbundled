...
extern void DSYSV(const char *uplo, const int *N, const int *NRHS, double *A,
...
static void update_model(quad_model *model, double *W, double *r, int M,
                         double *X, int inew, double fnew, int *iwork,
                         double *work) {
  double *q = model->q, *Q = model->Q;
  const double *x0 = model->x0;
...
static double cmodel_func(int n, const double *x, double *grad, void *data) {
  ...
  if (grad)
    eval_model_grad(&cmodel->model, x, grad);
  for (j = 0; j < n; ++j) {
    double siginv = 1.0 / sigma[j];
    double dx = (x[j] - xopt[j]) * siginv;
    val += (0.5 * rho) * dx * dx;
    if (grad)
      grad[j] += rho * dx * siginv;
  }
  return val;
}
...
nlopt_result cquad_minimize(int n, nlopt_func f, void *f_data, int m,
                            nlopt_func fc, void *fc_data_,
                            ptrdiff_t fc_datum_size, const double *lb,
                            const double *ub, /* bounds */
                            double *x, /* in: initial guess, out: minimizer */
                            double *minf, nlopt_stopping *stop,
                            nlopt_algorithm model_alg, double model_tolrel,
                            int model_maxeval) {
  ...
  if (!model.model.q || !model.model.Q) {
    ret = NLOPT_OUT_OF_MEMORY;
    goto done;
  }
  model.model.x0 = x0;
  ...
  for (i = 0; i < m; ++i)
    modelc[i].model.q = modelc[i].model.Q = 0;
  for (i = 0; i < m; ++i) {
    modelc[i].xopt = x;
    modelc[i].sigma = sigma;
    modelc[i].rho = 1;
    modelc[i].model = alloc_model(n);
    if (!modelc[i].model.q || !modelc[i].model.Q) {
      ret = NLOPT_OUT_OF_MEMORY;
      goto done;
    }
  ...
  *minf = HUGE_VAL;

  {
    ...
    *(stop->nevals_p)++;
    ...
    for (j = 0; j < n; ++j) {
      ...
      model.model.Q[j * n + j] =
          (fmp[1] + fmp[0] - 2 * model.model.q0) / (dx[j] * dx[j]);
      for (i = 0; i < m; ++i) {
        modelc[i].model.q[j] = (fcmp[1][i] - fcmp[0][i]) / (2 * dx[j]);
        modelc[i].model.Q[j * n + j] =
            (fcmp[1][i] + fcmp[0][i] - 2 * modelc[i].model.q0) /
            (dx[j] * dx[j]);
      }
    }
  }
  ...
  while (1) { /* outer iterations */
    ...
    if (ret != NLOPT_SUCCESS)
      goto done;
    if (++k > 1)
      memcpy(xprevprev, xprev, sizeof(double) * n);
    ...
    model_solution:
      ...
      reti = nlopt_minimize_constrained(
          model_alg, n, cmodel_func, &model, m, cmodel_func, modelc,
          sizeof(conservative_model), model_lb, model_ub, xcur, &min_model,
    ...
    got_new_xcur:
      ...
      *(stop->nevals_p)++;
      ...
      for (i = 0; i < m; ++i) {
        fcval_cur[i] = fc(n, xcur, NULL, fc_data + fc_datum_size * i);
        feasible_cur = feasible_cur && (fcval_cur[i] <= 0);
      ...
      for (i = 0; i < m; ++i)
        update_model(&modelc[i].model, W, r, M, X, iMnew, fcval_cur[i], iwork,
                     work);
      ...
      if (0 &&
          fabs(eval_model(&model.model, xcur) - fcur) > 1e-6 * fabs(fcur)) {
        if (cquad_verbose)
      ...
      if (fcur > gval)
        model.rho =
            MIN(10 * model.rho,
                1.1 * (model.rho + (fcur - gval) / wfunc(n, xcur, &model)));
      for (i = 0; i < m; ++i)
        if (fcval_cur[i] > gcval[i])
          modelc[i].rho =
              MIN(10 * modelc[i].rho,
                  1.1 * (modelc[i].rho + (fcval_cur[i] - gcval[i]) /
