...
/* +-----------------------------------------------------------------------+ */
/* Subroutine */ void
direct_direct_(fp fcn, doublereal *x, integer *n, doublereal *eps,
...
#define MY_ALLOC(p, t, n)                                                      \
   ...
   * tracking */
  {
    double *volatile _v = u;
    u = _v - 1;
  }
  {
    double *volatile _v = l;
    l = _v - 1;
  }
  {
  ...
  direct_dirheader_(logfile, &version, &x[1], n, eps, maxf, maxt, &l[1], &u[1],
                    algmethod, &MAXFUNC, &MAXDEEP, fglobal, fglper, ierror,
                    &epsfix, &iepschange, volper, sigmaper);
  /* +-----------------------------------------------------------------------+
   */
  ...
  /* +-----------------------------------------------------------------------+
   */
  direct_dirinit_(f, fcn, c__, length, &actdeep, point, anchor, &ifree, logfile,
                  arrayi, &maxi, list2, w, &x[1], &l[1], &u[1], minf, &minpos,
                  thirds, levels, &MAXFUNC, &MAXDEEP, n, n, &fmax, &ifeasiblef,
                  &iinfesiblef, ierror, fcn_data, jones, starttime, maxtime,
                  force_stop);
  /* +-----------------------------------------------------------------------+
   */
  ...
  if (*ierror < 0) {
    if (*ierror == -4) {
      if (logfile)
        fprintf(logfile,
                "WARNING: Error occurred in routine DIRsamplepoints.\n");
      goto cleanup;
    }
    if (*ierror == -5) {
      if (logfile)
    ...
    if (*ierror == -102)
      goto L100;
  } else if (*ierror == DIRECT_MAXTIME_EXCEEDED)
    goto L100;
  numfunc = maxi + 1 + maxi;
  actmaxdeep = 1;
  oldpos = 0;
  ...
  if (ifeasiblef > 0) {
    if (logfile)
      fprintf(logfile,
              "No feasible point found in %d iterations "
  ...
  for (t = tstart; t <= i__1; ++t) {
    ...
    direct_dirchoose_(anchor, s, &MAXDEEP, f, minf, *eps, epsabs, levels,
                      &maxpos, length, &MAXFUNC, &MAXDEEP, &MAXDIV, n, logfile,
                      &cheat, &kmax, &ifeasiblef, jones);
    /* +-----------------------------------------------------------------------+
     */
    ...
    if (*algmethod == 0) {
      ...
      if (*ierror == -6) {
        if (logfile)
          fprintf(logfile, "WARNING: Capacity of array S in DIRDoubleInsert "
                           "reached. Increase maxdiv.\n"
                           "This means that there are a lot of hyperrectangles "
                           "with the same function\n"
                           "value at the center. We suggest to use our "
                           "modification instead (Jones = 1)\n");
        goto cleanup;
      }
    }
    oldpos = minpos;
    ...
    for (j = 1; j <= i__2; ++j) {
      ...
      if (s[j - 1] > 0) {
        ...
        /* +-----------------------------------------------------------------------+
         */
        /* | If the current dept of division is only one under the maximal
         * allowed | */
        ...
        if (actdeep + 1 >= mdeep) {
          if (logfile)
            fprintf(logfile, "WARNING: Maximum number of levels reached. "
                             "Increase maxdeep.\n");
          *ierror = -6;
          goto L100;
        }
         ...
         * where  | */
        /* | we need to sample the function. After checking for errors, we then
         * do | */
        /* | the actual evaluation of the function, again followed by checking
         * for | */
        /* | errors. | */
        /* +-----------------------------------------------------------------------+
         */
        direct_dirsamplepoints_(c__, arrayi, &delta, &help, &start, length,
                                ...
                                minf, &minpos, &u[1], n, &MAXFUNC, &MAXDEEP,
                                &oops);
        if (oops > 0) {
          if (logfile)
            fprintf(logfile,
        ...
        /* +-----------------------------------------------------------------------+
         */
        /* | JG 01/22/01 Added variable to keep track of the maximum value
         * found.  | */
        /* +-----------------------------------------------------------------------+
         */
        direct_dirsamplef_(c__, arrayi, &delta, &help, &start, length, logfile,
                           f, &ifree, &maxi, point, fcn, &x[1], &l[1], minf,
                           &minpos, &u[1], n, &MAXFUNC, &MAXDEEP, &oops, &fmax,
        ...
        if (oops > 0) {
          if (logfile)
            fprintf(logfile,
                    "WARNING: Error occurred in routine DIRsamplef.\n");
          *ierror = -5;
          goto cleanup;
        }
        /* +-----------------------------------------------------------------------+
        ...
        /* +-----------------------------------------------------------------------+
         */
        direct_dirdivide_(&start, &actdeep_div__, length, point, arrayi, &help,
        ...
        /* +-----------------------------------------------------------------------+
         */
        direct_dirinsertlist_(&start, anchor, point, f, &maxi, length, &MAXFUNC,
                              &MAXDEEP, n, &help, jones);
        /* +-----------------------------------------------------------------------+
         */
    ...
    *ierror = jones;
    ...
    if (delta <= *volper) {
      *ierror = 4;
      if (logfile)
        fprintf(logfile,
                "DIRECT stopped: Volume of S_min is "
    ...
    if (delta <= *sigmaper) {
      *ierror = 5;
      if (logfile)
        fprintf(logfile,
                "DIRECT stopped: Measure of S_min "
                "= %g < %g.\n",
                delta, *sigmaper);
      goto L100;
    }
    ...
    if ((*minf - *fglobal) * 100 / divfactor <= *fglper) {
      *ierror = 3;
      if (logfile)
        fprintf(logfile,
                "DIRECT stopped: minf within fglper of global minimum.\n");
      goto L100;
    }
    /* +-----------------------------------------------------------------------+
     */
    ...
    if (increase == 1) {
      *maxf = numfunc + oldmaxf;
      if (ifeasiblef == 0) {
        if (logfile)
          fprintf(logfile,
                  "DIRECT found a feasible point.  The "
    ...
    if (numfunc > *maxf) {
      if (ifeasiblef == 0) {
        *ierror = 1;
        ...
        if (logfile)
          fprintf(logfile,
                  "DIRECT could not find a feasible point after %d function "
                  "evaluations.\n"
