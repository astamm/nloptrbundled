...
nlopt_result auglag_minimize(int n, nlopt_func f, void *f_data, int m,
  ...
  max_constraint_dim =
      MAX(nlopt_max_constraint_dim(d.m, fc), nlopt_max_constraint_dim(d.p, h));
  ...
  xcur = (double *)malloc(sizeof(double) *
                          (n + max_constraint_dim * (1 + n) + d.pp + d.mm));
  if (!xcur)
  ...
  *minf = HUGE_VAL;
  ...
  if (auglag_verbose) {
    nlopt_stop_log(stop,
                   "auglag: initial rho=%g\nauglag initial lambda=", d.rho);
    for (i = 0; i < d.pp; ++i)
      nlopt_stop_log(stop, " %g", d.lambda[i]);
    nlopt_stop_log(stop, "\nauglag initial mu = ");
    for (i = 0; i < d.mm; ++i)
      nlopt_stop_log(stop, " %g", d.mu[i]);
    nlopt_stop_log(stop, "\n");
  }
  ...
  do {
    ...
    ret = nlopt_optimize_limited(
        sub_opt, xcur, &fcur, stop->maxeval - *(stop->nevals_p),
    ...
    if (auglag_verbose) {
      nlopt_stop_log(stop,
                     "auglag %d: ICM=%g (%sfeasible), rho=%g\nauglag lambda=",
